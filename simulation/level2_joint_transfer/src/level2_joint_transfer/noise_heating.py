...
三个通道均为常数功率，注入能量随时间线性累积，系统随时间延长越发不稳定。
...
def constant_power(x_m, v_m_s, t_s, e_osc_J, power_w):
    """常数加热功率（三个通道的默认实现；e_osc_J 保留给自定义通道）。"""
    return power_w
...
def build_mean_heating(recoil_power_w=None, parametric_power_w=None,
    ...
    if parametric_power_w is not None:
        labels["parametric"] = {"kind": "constant_power", "power_w": float(parametric_power_w),
                                "power_uK_per_s": float(parametric_power_w) / microkelvin_to_joule(1.0)}
    ...
    return MeanHeating(
        ...
        parametric=None if parametric_power_w is None else partial(constant_power,
                                                                   power_w=float(parametric_power_w)),
...
def heating_from_config(cfg) -> MeanHeating | None:
    ...
    # 线性化：P = Γ_par × k_B·T_ref（T_ref 默认取初始系综温度）
    if noise.parametric_reference_temperature_uK is None:
        reference_temperature_uK = float(cfg.initial_ensemble.temperature_uK)
    else:
        reference_temperature_uK = float(noise.parametric_reference_temperature_uK)
