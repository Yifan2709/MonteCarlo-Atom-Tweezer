...
def _evaluate_chunk(args):
    ...
    spec, states, physics, dt_s, hold_s, alignment_offset_m, work_energy, offset, size = args
...
def evaluate_waveform_on_states(spec: dict, states: dict, physics: dict, dt_s: float,
    ...
    for shot in range(x0.size):
        time, x, v, _a = velocity_verlet_time_dependent(total_force, mass, float(x0[shot]), float(v0[shot]), times)
...
def run_validation(cfg: Level2Config, specs: dict, states: dict, physics: dict):
    ...
    for name, spec in specs.items():
        ...
        results[name] = evaluate_waveform_parallel(
            ...
            cfg.integration.post_transfer_hold_s, work_energy=True)
    names = list(specs)
    comparisons = {}
