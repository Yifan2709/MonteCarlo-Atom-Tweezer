...
最终统计审查还修正了两个汇总问题：当上一轮已无原子时，条件损失应未定义，而不能报告100%；合并种子后的条件损失及能量分位数必须由总计数和全部幸存者能量重算，不能复制某一条代表记录。`analyze.py`现已直接从计数/NPZ重算。冻结计算的原始JSON中`conditional_loss=1`在此前已经全损失的行只是旧占位值，保留以供审计；最终CSV中这些位置为空值。这不改动动力学、存活计数或速度边界。
