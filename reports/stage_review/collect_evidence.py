"""从仓库现有输出生成 v9 报告及可追溯数值，不运行或拟合 Monte Carlo。"""
...
STEM = "AOD_SLM_MonteCarlo_levels2c_3_4_5_2026-09-14_v9"
SOURCES = []
...
def main():
    l3 = PROJECT / "outputs/level3_3d_transport_lensing/demo"
    l4 = PROJECT / "outputs/level4_end_to_end_roundtrip/demo"
    l5 = PROJECT / "outputs/level5_finite_pulse_irb/demo"
    a, b, c = [read_json(p / "metrics.json") for p in (l3, l4, l5)]
    coarse = read_csv(l3 / "coarse_scan.csv")
    repeated = read_csv(l4 / "repeated_round_metrics.csv")
    for pattern in ("configs/level3_*.yaml", "configs/level4_*.yaml", "configs/level5_*.yaml",
                    "src/level3_3d_transport_lensing/*.py", "src/level4_roundtrip_coherence/*.py",
                    "src/level5_finite_pulse_irb/*.py", "src/simulation_workflow/*.py"):
        for p in sorted(PROJECT.glob(pattern)):
            source(p)
    source(l5 / "RESULT_STATUS.md")
    for rel in ("README.md", "run_simulation.py", "2403.12021v4.md",
                "reports/stage_review/build_report.mjs",
                "reports/stage_review/restore_chart_workbooks.py",
                "simulation/level2_joint_transfer/tests/test_level5_irb_regression.py",
                "simulation/level2_joint_transfer/tests/test_simulation_workflow.py"):
        source(ROOT / rel)
    source(ROOT / "reports/stage_review/output/AOD_SLM_MonteCarlo_step_by_step_2026-09-12_v8.pptx")
    source(Path(__file__))
    verification = ROOT / "reports/stage_review/.build/v9-verification.json"
    checks = read_json(verification) if verification.is_file() else {"status": "not_recorded"}
    data = {
        "generated_utc": datetime.now(timezone.utc).isoformat(),
        "base_commit": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=ROOT, text=True).strip(),
        "provenance": "Existing demo outputs; this report update did not rerun production MC or RB.",
        "level3": {"physics": a["physics"], "lensing": a["lensing"],
                   "validation": a["validation"]["summaries"], "coarse": coarse,
                   "compensation": a["compensation"]},
        "level4": {"single_round": b["single_round"]["summaries"],
                   "contrast": b["single_round"]["contrast"], "repeated": repeated},
        "level5": {"channel": c["channel"]["conditions"],
                   "reference_rb": c["reference_rb"],
                   "historical_irb_invalid": c["interleaved_rb"],
                   "irb_status": "invalid_historical_multisite_results_require_rerun",
                   "current_fixed_N_standard_ratio_allowed": False},
        "verification": checks,
        "sources": SOURCES,
    }
    v3 = a["validation"]["summaries"]
    v4 = b["single_round"]["summaries"]
    phase = b["single_round"]["contrast"]
    channels = c["channel"]["conditions"]
    round10 = next(x for x in repeated if x["protocol"] == "protocol_a" and x["round"] == "10")
    end40 = next(x for x in repeated if x["protocol"] == "protocol_a_long_tail" and x["round"] == "40")
    t3 = "\n".join(f"| {k.replace('|', ' / ')} | {v['retained']}/{v['shots']} | {v['retention_fraction']:.4f} | [{v['wilson_low']:.4f}, {v['wilson_high']:.4f}] |" for k,v in v3.items())
    t4 = "\n".join(f"| {k} | {v['successes']}/{v['shots']} | {v['probability']:.4f} | [{v['wilson_low']:.4f}, {v['wilson_high']:.4f}] |" for k,v in v4.items())
    tphase = "\n".join(f"| {k} | {phase['protocol_a_nominal_lensing|'+k]['contrast']:.6f} |" for k in ("none", "spin_echo", "xy4_per_long_move"))
    t5 = "\n".join(f"| {k.replace('|', ' / ')} | {channels[k]['survival']:.4f} | {channels[k]['f_avg']:.6f} |" for k in (
        "protocol_a|none|nominal", "protocol_a|spin_echo|nominal", "protocol_a|xy4_per_long_move|nominal", "protocol_a|xy16|nominal"))
    text = f"""# AOD–SLM Monte Carlo 综合阶段报告 v9

更新日期：2026-09-14。基于代码提交 `{data['base_commit'][:7]}` 加本轮工作区修改。
...
已有粗扫描 CSV 为 {len(coarse)} 个格点的子集，应以 CSV 中实际参数为准。
...
重复 CSV：Protocol A 第 10 轮仍为 {round10['absorbing_success']}/500 成功，
C_none={float(round10['contrast_none']):.4f}，C_echo={float(round10['contrast_spin_echo']):.4f}，C_XY4={float(round10['contrast_xy4']):.4f}。
长尾第 40 轮为 {end40['absorbing_success']}/128 成功。
...
本轮验证记录：`{json.dumps(checks, ensure_ascii=False)}`。
...
"""
    ...
    (OUT / f"{STEM}.evidence.json").write_text(json.dumps(data, ensure_ascii=False, indent=2), encoding="utf-8")
    print(f"Wrote report and evidence: {len(SOURCES)} sources")
